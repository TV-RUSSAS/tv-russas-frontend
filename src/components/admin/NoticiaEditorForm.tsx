...
function SocialEmbedNodeView({ node, deleteNode }: NodeViewProps) {
  ...
  return (
    <NodeViewWrapper
      className="social-embed-editor-block"
      style={{
        margin: "20px 0",
...
function EditorToolbar({
  ...
  const [imageCreditos, setImageCreditos] = useState("");
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        fontMenuRef.current &&
        !fontMenuRef.current.contains(event.target as HTMLElement)
      ) {
        setShowFontMenu(false);
      }
      if (
        colorMenuRef.current &&
        !colorMenuRef.current.contains(event.target as HTMLElement)
      ) {
  ...
  const handleInsertEmbed = () => {
    ...
    if (platform === "unknown") {
      alert(
        "Domínio não permitido por segurança. Use apenas links confiáveis do YouTube, Instagram ou Facebook.",
      );
      return;
    }
  ...
  const handleInsertEditorImage = async () => {
    if (!imageFile) return;
    ...
    try {
      ...
      const res = await authFetch("/admin/upload-image", {
        method: "POST",
        body: formData,
      });
      if (!res.ok) {
        ...
        throw new Error(`Falha ao subir imagem: ${res.status} - ${text}`);
      }
  ...
  return (
    <div className="ed-toolbar-sticky">
      ...
      <button
        ...
        title="Inserir Vídeo do YouTube / Rede Social no Texto"
        className="ed-btn"
          ...
          >
            ...
            <div style={{ marginBottom: "24px" }}>
              ...
              <input
                ...
                value={imageCreditos}
                onChange={(e) => setImageCreditos(e.target.value)}
                style={{
                  width: "100%",
                  padding: "12px",
                  background: "var(--c-bg)",
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: "8px",
                  cursor:
                    imageFile && !isUploadingImage ? "pointer" : "not-allowed",
                  fontWeight: 600,
                }}
              >
      ...
      {showEmbedModal && typeof document !== "undefined" && createPortal(
        ...
        >
          ...
          >
            ...
            >
              <i className="fas fa-video" style={{ color: "#ff9800" }} />{" "}
              Inserir Vídeo na Matéria
            </h3>
            ...
            >
              <div>
                ...
                >
                  Link / URL do YouTube ou Vídeo Direto
                </label>
                <input
                  type="text"
                  value={embedUrl}
              ...
              <div>
                ...
                >
                  Crédito / Fonte do Vídeo (Opcional)
                </label>
                <input
                  type="text"
                  value={embedCredit}
...
export function NoticiaEditorForm({
  initialData,
  mode,
  initialCategorias,
  initialColunistas,
  ...
  const [showVideoCapaModal, setShowVideoCapaModal] = useState(false);
  ...
  const [colunistaId, setColunistaId] = useState(
    initialData?.colunistaId || "",
  );
  ...
  const [destaquesAtuais, setDestaquesAtuais] = useState<
    { id: string; titulo: string; capaUrl: string; publicadoEm: string }[]
  >([]);
  ...
  const extractFonte = (html: string) => {
    const match = html.match(
      /<p>\s*(?:<strong>|<b>)?\s*Fonte\s*:\s*([^<]+?)\s*(?:<\/strong>|<\/b>)?\s*<\/p>/i,
    );
  ...
  const removeFonte = (html: string) => {
    return html.replace(
      /<p>\s*(?:<strong>|<b>)?\s*Fonte\s*:\s*([^<]+?)\s*(?:<\/strong>|<\/b>)?\s*<\/p>/gi,
      "",
    );
  };
  ...
  const extractPublicadoPor = (html: string) => {
    ...
    const match = html.match(
      /<p>\s*(?:<strong>|<b>)?\s*Publicado\s+por\s*:\s*([^<]+?)\s*(?:<\/strong>|<\/b>)?\s*<\/p>/i,
    );
  ...
  const removePublicadoPor = (html: string) => {
    return html.replace(
      /<p>\s*(?:<strong>|<b>)?\s*Publicado\s+por\s*:\s*([^<]+?)\s*(?:<\/strong>|<\/b>)?\s*<\/p>/gi,
      "",
    );
  };
  ...
  const [creditosFotoText, setCreditosFotoText] = useState(
    initialData?.creditosFoto || "",
  );
  const [descricaoCapaText, setDescricaoCapaText] = useState(
    initialData?.descricaoFoto || "",
  );
  const [creditosVideoText, setCreditosVideoText] = useState(
    initialData?.creditosVideo || "",
  );
  const [descricaoVideoText, setDescricaoVideoText] = useState(
    initialData?.descricaoVideo || "",
  );
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder: "Escreva a matéria com maestria jornalística...",
      }),
      CharacterCount,
      Color,
      TextStyle,
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      localStorage.setItem(
        `draft-noticia-${mode}-${initialData?.id || "new"}`,
    ...
    if (mode === "create") {
      ...
      if (saved) {
        try {
          ...
          if (
            confirm("Encontramos um rascunho não salvo. Deseja restaurá-lo?")
          ) {
            setTimeout(() => {
  ...
  const executeSubmit = async () => {
    ...
    if (content === "<p></p>" || !content.trim()) {
      setError(TEXTS.admin.newsContentEmpty);
      return;
    }
    if (mode === "create" && !capa) {
      setError(TEXTS.admin.coverRequired);
      return;
    }
    ...
    try {
      ...
      if (replaceFeaturedId)
        formData.append("replaceFeaturedId", replaceFeaturedId);
      ...
      formData.append("creditosFoto", creditosFotoText);
      formData.append("descricaoFoto", descricaoCapaText);
      formData.append("creditosVideo", creditosVideoText);
      formData.append("descricaoVideo", descricaoVideoText);

      const url =
        mode === "edit"
          ? `/admin/noticias/${initialData?.id}`
          : "/admin/noticias";
      const method = mode === "edit" ? "PUT" : "POST";

      const token = sessionStorage.getItem("accessToken");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:3001"}/api${url}`,
        {
          method,
    ...
    <>
            ...
            <h2 className="cms-page-title">
              {mode === "create" ? (
                ...
                <>
                  ...
                  />{" "}
                  {TEXTS.admin.editMatter}
                </>
              )}
            </h2>
            <p
              className="cms-page-subtitle"
              style={{ display: "flex", alignItems: "center", gap: "12px" }}
            >
              <span>
                {mode === "edit"
                  ? TEXTS.admin.editMode
                  : TEXTS.admin.createMode}
              </span>
              {saveStatus === "saving" && (
                <span
                  style={{
              ...
              {saveStatus === "saved" && (
                ...
                >
                  <i className="fas fa-check" /> {TEXTS.admin.draftSaved}
                </span>
              )}
            </p>
          </div>
          ...
          >
            ...
            <div className="cms-table-card" style={{ padding: "24px" }}>
              ...
              >
                ...
                >
                  {TEXTS.admin.feedClassification}
                </div>
                  ...
                  <div>
                    <div
                      style={{
                        fontWeight: "600",
                        fontSize: "13px",
                        color: !featured ? "#fff" : "var(--c-secondary)",
                      }}
                    >
                      {"Última Notícia"}
                    </div>
                    <div style={{ fontSize: "10px", color: "var(--c-muted)" }}>
                      {TEXTS.admin.standardFeedNews}
                    </div>
                  </div>
                </div>
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      {TEXTS.admin.bannerHighlight}
                    </div>
                    <div style={{ fontSize: "10px", color: "var(--c-muted)" }}>
                      {TEXTS.admin.bannerHighlightSub}
                    </div>
                  </div>
                </div>
              </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    <i className="far fa-save" style={{ marginRight: "8px" }} />{" "}
                    {TEXTS.actions.save}
                  </>
                )}
              </button>
            ...
            <div className="cms-table-card" style={{ padding: "24px" }}>
              ...
              <div style={{ marginBottom: "16px" }}>
                ...
                {isVertical && (
                  ...
                  >
                    ...
                    />
                    <div>
                      <strong>Imagem vertical detectada.</strong> Ela será
                      exibida inteira dentro da matéria, mas poderá sofrer
                      cortes nas miniaturas da página inicial.
                    </div>
                  </div>
                )}
                <div
                ...
                >
                  ...
                  <span>
                    Use imagens horizontais, preferencialmente{" "}
                    <strong>1200x675</strong>.
                  </span>
                </div>
              </div>
              ...
              <div style={{ marginBottom: "16px" }}>
                ...
                {videoPreview ? (
                  ...
                  >
                    ...
                    >
                      <button
                        type="button"
                        onClick={() => setShowVideoCapaModal(true)}
                        style={{
                ...
                >
                  <i className="fas fa-info-circle" style={{ flexShrink: 0 }} />
                  <span>
                    O vídeo aparecerá na capa dentro da matéria.
                    Nos feeds do portal, será exibida a Imagem de Capa normal.
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="cms-table-card" style={{ padding: "24px" }}>
              ...
              <div className="cms-form-group">
                <label className="cms-label">
                  {TEXTS.admin.category} <span>*</span>
                </label>
                <select
              ...
              <div className="cms-form-group">
                ...
                <input
                  ...
                  placeholder="ex: russas, política"
                />
              </div>
              <div className="cms-form-group">
                ...
                <input
                  ...
                  value={fonteText}
                  onChange={(e) => {
                    setFonteText(e.target.value);
                    setSaveStatus("saving");
                  }}
                  placeholder="ex: Assessoria, TV Russas"
                />
              </div>
            </div>
            ...
            <div className="cms-table-card" style={{ padding: "24px" }}>
              ...
              >
                  ...
                  <div style={{ display: "flex", flexDirection: "column" }}>
                    ...
                    >
                      <span style={{ color: "#202124" }}>
                        {"https://tvrussas.com.br"}
                      </span>
                      <span style={{ fontSize: "10px", color: "#70757a" }}>
                        {"›"}
                      </span>
                      <span>{"noticia"}</span>
                      {slug && (
                ...
                >
                  {resumo ||
                    "Resumo impactante da matéria que aparecerá no Google. É recomendado ter entre 120 e 160 caracteres para garantir o melhor alcance orgânico e clareza nos resultados de pesquisa."}
                </div>
              </div>
            </div>
          </div>
          ...
          >
            ...
            >
              <p style={{ marginBottom: "20px" }}>
                {TEXTS.admin.bannerMaxAlert}{" "}
                <strong>{"3 notícias simultâneas"}</strong>
                {
                  ". Selecione qual matéria sairá do topo para dar lugar à sua nova postagem:"
                }
              </p>
                ...
                {destaquesAtuais.map((noticia) => (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {new Date(noticia.publicadoEm).toLocaleDateString(
                          "pt-BR",
                        )}
                      </span>
                    </div>
      ...
      {showVideoCapaModal && (
        <div
          ...
          onClick={() => setShowVideoCapaModal(false)}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            style={{
